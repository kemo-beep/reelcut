...
func (r *transcriptionRepository) Create(ctx context.Context, t *domain.Transcription) error {
	query := `INSERT INTO transcriptions (id, video_id, language, status, error_message, word_count, duration_seconds, confidence_avg)
		VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`
	_, err := r.pool.Exec(ctx, query, t.ID, t.VideoID, t.Language, t.Status, t.ErrorMessage, t.WordCount, t.DurationSeconds, t.ConfidenceAvg)
	return err
}
...
func (r *transcriptionRepository) GetByID(ctx context.Context, id string) (*domain.Transcription, error) {
	...
	err := r.pool.QueryRow(ctx, query, id).Scan(&t.ID, &t.VideoID, &t.Language, &t.Status, &t.ErrorMessage, &t.WordCount, &t.DurationSeconds, &t.ConfidenceAvg, &t.CreatedAt, &t.UpdatedAt)
	if err != nil {
		return nil, err
	}
	return &t, nil
...
func (r *transcriptionRepository) CreateWithSegments(ctx context.Context, t *domain.Transcription, segments []*domain.TranscriptSegment) error {
	...
	query := `INSERT INTO transcriptions (id, video_id, language, status, error_message, word_count, duration_seconds, confidence_avg)
		VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`
	if _, err := tx.Exec(ctx, query, t.ID, t.VideoID, t.Language, t.Status, t.ErrorMessage, t.WordCount, t.DurationSeconds, t.ConfidenceAvg); err != nil {
		return err
	}
